...
class MultiHeadAttention:
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None):
        ...
        attn_outputs = self._concat_heads(attn_outputs)

        # Project the attention outputs
        # (N, L, embed_dim) -> (N, L, embed_dim)
        output = self.out_proj.forward(attn_outputs)
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = np.tile(np.expand_dims(key_padding_mask,(1,2)),(1,self.num_heads,self.L,1))
        
        
        # Expand attn_mask to (1, 1, L, S) and broadcast to (N, H, L, S)

    
        attention_mask = np.tile(np.expand_dims(attn_mask,(0,1)),((self.N,self.num_heads,1,1)))
