...
class ScaledDotProductAttention:
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        if mask is not None:
            eps_array = np.full(scaled_dot_product.shape,-self.eps)*mask
            scaled_dot_product = eps_array + scaled_dot_product
    ...
    def backward(self, d_output):
        ...
        d_Q = (d_scaled_dot_product/np.sqrt(self.Q.shape[-1]))@self.K
        # (N, ..., H, L, S) @ (N, ..., H, L, E) -> (N, ..., H, S, E)
        d_K = (d_scaled_dot_product/np.sqrt(self.Q.shape[-1])).swapaxes(-1,-2)@self.Q
