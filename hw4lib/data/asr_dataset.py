...
class ASRDataset(Dataset):
    def __init__(
        ...
        self.fbank_dir   = config["root"]+"/"+self.partition+"/fbank"
        
        # TODO: Get all feature files in the feature directory in sorted order  
        self.fbank_files = os.listdir(self.fbank_dir)
        
        self.fbank_files.sort()
        ...
        if self.partition != "test-clean":
            ...
            self.text_dir   = config["root"]+"/"+self.partition+"/text"
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.partition != "test-clean":
            # TODO: Get transcripts for non-test partitions
            shifted = torch.tensor(self.transcripts_shifted[idx]).to(torch.long)
            golden  = torch.tensor(self.transcripts_golden[idx]).to(torch.long)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.tensor([x.shape[1] for x in batch_feats]).to(torch.int) # B
        ...
        padded_shifted, padded_golden, lengths = None, None, None
        if self.partition != "test-clean":
            ...
            shifted_transcripts, golden_transcripts = [x[1] for x in batch], [x[2] for x in batch]
        ...
        return padded_feats, padded_shifted, padded_golden, feat_lengths, lengths
